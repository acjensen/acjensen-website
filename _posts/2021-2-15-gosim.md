...
The pendulum's governing equation is \\( \ddot{\theta} + \frac{-g}{L}\theta = 0 \\). We can break the second order differential equation into two first-order differential equations by defining system states \\(s_1\\) and \\(s_2\\).
